...
public:
  ...
  CourseSchedule(bool rmcon) : remove_conflicts_(rmcon), num_courses_(0) {}
  ~CourseSchedule();
...
public:
  ...
  ~ScheduleManager(); // deallocate best_schedule if not nullptr

  CourseSchedule * best_schedule(const std::string & filename);
};
