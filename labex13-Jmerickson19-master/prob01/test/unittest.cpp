...
TEST(Exception, constructor)
{
  ...
  ASSERT_EQ(your_ex.message(), "My exception message") << "Either your accessor message() was not working correctly, or your constructor did not set the message correctly";
}

TEST(Exception, mutator)
{
  Exception your_ex("Default message");
  your_ex.set_message("Mutator exception message");
  ASSERT_EQ(your_ex.message(), "Mutator exception message") << "Either your accessor message() was not working correctly, or your mutator set_message() did not set the message correctly";
}

TEST(IndexOutOfRangeException, constructor)
{
  IndexOutOfRangeException your_ex(77, 44);
  ASSERT_THAT(your_ex.message(), HasSubstr("77")) << "Your constructor did not set the index correctly";
  ASSERT_THAT(your_ex.message(), HasSubstr("44")) << "Your constructor did not set the array size correctly";
  ASSERT_THAT(your_ex.message(), HasSubstr("0 and 43")) << "Your constructor did not set the correct value range.";
}

TEST(InvalidArrayLengthException, constructor)
{
  InvalidArrayLengthException your_ex(-22);
  ASSERT_THAT(your_ex.message(), HasSubstr("-22)")) << "Your constructor did not show the invalid index value";
  ASSERT_THAT(your_ex.message(), HasSubstr("positive integer")) << "Your constructor did not emphasize positive integers";
}
...
TEST(ArrayDouble, Display)
{
	...
	your_array[2] = 0.6;
		SIMULATE_SIO("", your_array.display(), {
			ASSERT_THAT(your_output, ContainsRegex("1\\.00.*2\\.00.*0\\.6")) << "The function should display all elements of the array: 1.0, 2.0, 0.6";
		})
}
